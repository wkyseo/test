/**
 * Created by junxie on 2016/12/13.
 */
'use strict';
...
function createHandlers(callback) {
  ...
  var handler = {
    ...
    touchStart: function(event) {
      ...
      state.touches++;
      state.touchTimeout = setTimeout(endTouch.bind(event.target, state), 250);  //250
    ...
    touchEnd: function(event) {
      ...
      if (Math.abs(deltaX) < 5 && Math.abs(deltaY) < 5) {
        ...
        event.preventDefault();
        callback(event);
        state.touchTimeout = setTimeout(endTouch.bind(event.target, state), 50);  //425, 原插件425是为防止短时间内多次触发，但是连续按不流畅，所以改成50，如：数字键盘
      }
    },
