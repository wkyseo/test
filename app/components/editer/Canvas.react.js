...
import {Hit_From} from '../../constants/BeginGuideSteps';
class Canvas extends Component {
  ...
  render() {
    return (
      <div>
        <Link to="/app" className='home'><div className='back-to-entrance' ref={(node)=>{this.backNode = node;}}><img src='img/icon-back.png' /></div></Link>
        <div className="editer-canvas canvas-normal" id="editer-canvas-absolute"
        ...
        for (let i = 0; i < valueTxt.length; i++) {
          if (valueTxt[i].closest('.node-port-output').id.split('-')[1] == bundle.port) {
            if (typeof bundle.value == 'boolean') {
              ...
              valueTxt[i].textContent = bundle.value == true ? 'Y' : 'N';
  ...
  showObjectStyle(ele) {
    let core = ele.closest('.core');
    core.style.width = window.innerWidth > 1023 ? '16px' : '10px';  // if phone, using 10px
    core.style.height = window.innerWidth > 1023 ? '16px' : '10px';
    core.closest('.node-port-outputs').style.marginLeft = '0';
    ele.style.width = window.innerWidth > 1023 ? '10px' : '6px';  // if phone, using 6px
    ele.style.height = window.innerWidth > 1023 ? '10px' : '6px';
  ...
  showBooleanStyle(ele, flag) {
    ...
    core.style.width = window.innerWidth > 1023 ? '16px' : '10px';
    core.style.height = window.innerWidth > 1023 ? '16px' : '10px';
  ...
  forbidNodeDrag() {
    //禁用节点拖拽
    this.refs.editerCanvas.querySelectorAll('.node-body').forEach((element)=>{
      element.classList.remove('node-draggable');
    });
  }

  forbidPortDrag() {
    //禁用端口拖拽
    this.refs.editerCanvas.querySelectorAll('.node-port').forEach((element)=>{
      element.classList.remove('node-port');
    });
  }
  ...
  guideStartCallback(stepAllData) {
    if(stepAllData && stepAllData.stepData && stepAllData.stepData.param) {
      ...
      let hitFrom = stepAllData.stepData.from;
      if(index === 3 || index === 8) {
        setTimeout(()=>{
          this.refs.magnifer.style.display = 'none';

          //使画布不能拖拽
          this.refs.editerCanvas.classList.remove('canvas-normal');
        },0);
      }
      ...
      //点击连线的步骤
      if(hitFrom === Hit_From.canvasLine) {
        this.forbidNodeDrag();
        this.forbidPortDrag();
      }
    }
  ...
  componentWillUnmount() {
    ...
    if (editerDiv != null && guideStore.isSkippedGuide !== true) {
