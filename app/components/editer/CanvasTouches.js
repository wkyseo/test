...
class PortTouchHandler extends TouchHandler {
  ...
  onMove(e) {
    ...
    if (this.willConnect) {
      ...
      //setPortTouchInfo for multiTouch
      NodeUtils.setPortTouchInfo(this.lists.enlargedList, this.lists.shrinkedList, this.enlargedNode, this.enlargedPort);
    }
  }
  ...
  onEnd() {
    ...
    if (this.willConnect) {
      NodeUtils.setIsPortTouch(false);
      this.willConnect = false;
      if (this.canDraw) {
        this.drawRealLine();
        this.canDraw = false;
...
class CanvasTouchHandler extends TouchHandler {
  ...
  onStart(e) {
    ...
    let tmp = e.target;
    if ((tmp.classList.contains('canvas-normal') || tmp.tagName == 'svg') && this._fingers(e) == 1) {
      ...
      console.log('zoomming,', this._fingers(e));

      //multiTouch for NodePort  will be canceled
      if(NodeUtils.getIsPortTouch()) {
        let portTouchObj = NodeUtils.getPortTouchInfo();
        if(portTouchObj instanceof Object) {
          NodeUtils.clearEnlargedPort(portTouchObj.enlargedList, portTouchObj.shrinkedList);
          this.container.refs.fakewire.style.display = 'none';
          if(portTouchObj.enlargedNode && portTouchObj.enlargedPort) {
            NodeUtils.clearHighlightNode(portTouchObj.enlargedNode, portTouchObj.enlargedPort, this.container.refs.fakewire);
          }
        }
      }
      ...
      //canvas offsetRect set when multiTouch for canvasTouch
