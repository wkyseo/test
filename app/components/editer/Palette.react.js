...
class NodeTouchHandler extends TouchHandler {
  ...
  onStart(e) {
    if ( e.target.closest('.node-preview') && e.touches.length == 1) {
      ...
      document.body.appendChild(this.draggingObject);
    }
...
class Palette extends Component {
  constructor() {
    ...
    this._lastOpenCameraTime = 0;
    this._currentOpenCameraTime = 0;
    this._minOpenCameraInterval = 1000; // 1 second
    this.state = {
      category: 'common',
      reRender: false,
      tabList: [],
  ...
  renderTabs(){
    ...
    for(let tabName in this.state.tabList){
      if (this.state.tabList[tabName] !== 'hidden'){
    ...
    return (<ul className='tabs ' ref='tabNames'>{tabs}</ul>);
  }
  ...
  */
  ...
  limitTabCamera() {
    if(this.state.showVideo) {
      console.log('limitTabCamera close camera');
      return true;
    }

    this._currentOpenCameraTime = new Date().getTime();
    let openInterval = this._currentOpenCameraTime - this._lastOpenCameraTime;
    console.log('limitTabCamera openInterval:', openInterval);
    if(openInterval < this._minOpenCameraInterval) {
      console.log('limitTabCamera openInterval less than _minOpenCameraInterval:', this._minOpenCameraInterval);
      return false;
    }
    console.log('limitTabCamera openInterval large than _minOpenCameraInterval:', this._minOpenCameraInterval);
    this._lastOpenCameraTime = this._currentOpenCameraTime;
    return true;
  }

  //display or hide video camera
  onToggleCameraFrameDisplay() {
    let enableTab = this.limitTabCamera();
    if(false == enableTab) {
      return;
    }
  ...
  guideStartCallback(stepAllData) {
    if(stepAllData && stepAllData.stepData && stepAllData.stepData.param) {
      ...
      if(index === 15) {
        this.refs.mainPaletteTab.classList.add('editer-palette-mask');
      }
    }
  }
  ...
  guideEndCallback(stepAllData) {
    if(stepAllData && stepAllData.stepData && stepAllData.stepData.param) {
      //let index = parseInt(stepAllData.stepData.param.index);
      /*if (guideStore.guide.isCanceled() && this.refs.magnifer) {
        this.refs.magnifer.style.display = 'block';
      }*/
    }
  }
