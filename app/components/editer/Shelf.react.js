...
class Shelf extends Component {
  ...
  renderNodes() {
    ...
    if (this.inspector_PC == false || this.state.connectedType == '') {
      return (
        <div {...tapOrClick(this.chooseConnectType.bind(this))} className='noConnection'>
          <div className='not-connected-icon'>
            <img className='connected-hardware' src='img/icon-link.png'/>
          </div>
          <div className='not-connected-text'>{languages.getTranslation('connect-neuron-blocks-text')}</div>
        </div>
      );
    }
  ...
  chooseConnectType() {
    UIActions.openConnectionTypeDialog();
  }
  ...
  guideStartCallback(stepAllData) {
    if(stepAllData && stepAllData.stepData && stepAllData.stepData.param) {
      ...
      if(index === 7) {
        this.shelfNode.classList.add('guide-masked');
      }
      if(index === 8) {
        this.shelfNode.classList.add('untouchable');
      }
      if(index === 15) {
        this.shelfNode.classList.add('editer-shelf-mask');
      }
    }
  }

  guideEndCallback(stepAllData) {
    if (guideStore.guide.isCanceled()) {
      this.refs.classList.remove('guide-masked');
    }
    if(stepAllData && stepAllData.stepData && stepAllData.stepData.param) {
      let index = parseInt(stepAllData.stepData.param.index);
      if(index === 7) {
        this.shelfNode.classList.remove('guide-masked');
      }
      if(index === 8) {
        this.shelfNode.classList.remove('untouchable');
      }
  ...
  componentDidUpdate() {
    if ((this._firstTimeEnter) && (this.inspector_PC == false || this.state.connectedType == '')) {
      this._firstTimeEnter = false;
      UIActions.openConnectionTypeDialog();
    }
  }
