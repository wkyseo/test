...
class ColorCheckNode extends PureComponent {
  ...
  componentDidMount() {
    if(this.props.id){
      ...
      let sampleColor = nodeStore.getNodeConfigs(this.props.id).sampleColor;
      if(sampleColor.defaultValue === null){
        this.takeColor();
      }else {
        this.setState({
          R:sampleColor.defaultValue.R,
          G:sampleColor.defaultValue.G,
          B:sampleColor.defaultValue.B
        });
      }
