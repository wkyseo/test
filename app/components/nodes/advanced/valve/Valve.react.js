...
class ValveNode extends PureComponent {
  constructor() {
    super(...arguments);
  }
  ...
  renderPreview() {
    return (
      <div className={'node-preview node-preview-palette node-type-' + this.props.info.name} data-type={this.props.info.name} data-category={this.props.info.props.category}>
        <img className="node-preview-icon" src='./img/icon-valve.png' />
        <span className="node-preview-name">{languages.getTranslation('valve')}</span>
      </div>
    );
  }
    ...
    return (
      <div className={'node-actual node-type-' + this.props.info.name} id={this.props.id} style={{
        ...
        <div className='node-body node-draggable hide-config' style={{
          backgroundImage: 'url("img/icon-valve-onNode.png")'}}>
        </div>
