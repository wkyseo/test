      ...
      <div className={'node-actual node-type-' + this.props.info.name} id={this.props.id} style={{
        ...
        <div className='node-body node-draggable'>
           <div className='node-actual-body'>
            <span className="node-button-panel-title">{languages.getTranslation('counter')}</span>
            <span className="node-button-panel-value">{this.state.value}</span>
            <span className="node-button-panel-btn hide-config" {...tapOrClick(this.onReset.bind(this))}>{languages.getTranslation('reset')}</span>
          </div>       
        </div>
